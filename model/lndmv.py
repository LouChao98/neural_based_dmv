...
from utils.functions import (calculate_uas, cp2torch, torch2cp, use_torch_in_cupy_malloc,
                             print_to_file, get_tag_id_converter, get_init_param_converter)
from utils.runner import Runner, Model, Logger, RunnerOptions
from utils.data import ConllDataset, Vocab, BLLIP_POS, WSJ_POS
...
class LNMDVModelOptions(RunnerOptions, DMVOptions, NeuralMOptions):
    ...
    pos_emb_path: str = 'data/bllip_vec/posvectors.npy'  # if not given, random init. for use_emb_as_w=True.
...
class LNDMVModel(Model):
    ...
    def build(self):
        ...
        word_idx = cp.arange(2, len(self.r.train_ds.word_vocab))
...
class LNDMVModelRunner(Runner):
    ...
    def load_ds(self):
        ...
        self.o.max_len = 10
        self.o.num_tag = len(BLLIP_POS) + len(word_vocab) - 2
        self.o.num_lex = len(word_vocab) - 2
